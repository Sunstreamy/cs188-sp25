...
def joinFactors(factors: List[Factor]):
    ...
    for factor in factors:
        allUnconditioned = allUnconditioned | set(factor.unconditionedVariables())
    # 条件变量
    allConditioned = set()
    for factor in factors:
        allConditioned = allConditioned | set(factor.conditionedVariables())
    allConditioned -= allUnconditioned

    variableDomainDict = factors[0].variableDomainsDict()
    newFactor = Factor(list(allUnconditioned), list(allConditioned), variableDomainDict)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        Uncond = set(factor.unconditionedVariables()) - {eliminationVariable}
        Cond = set(factor.conditionedVariables())

        variableDomainsDict = factor.variableDomainsDict()
        newFactor = Factor(list(Uncond), list(Cond), variableDomainsDict)

        for assignment in newFactor.getAllPossibleAssignmentDicts():
            total = 0.0
            for v in variableDomainsDict[eliminationVariable]:
                extended = assignment.copy()
                extended[eliminationVariable] = v
