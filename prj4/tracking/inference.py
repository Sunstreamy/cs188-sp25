...
def constructBayesNet(gameState: hunters.GameState):
    ...
    mxDistance = (X_RANGE - 1) + (Y_RANGE - 1)
    mxObs = mxDistance + MAX_NOISE
    variableDomainsDict[OBS0] = range(0, mxObs + 1)
    variableDomainsDict[OBS1] = range(0, mxObs + 1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        ...
        for elimVar in eliminationOrder:
            factorsWithoutEliVar, joinedFactor = joinFactorsByVariable(factors, elimVar)

            if joinedFactor is None or len(joinedFactor.unconditionedVariables()) == 1:
                factors = factorsWithoutEliVar
                continue

            reduced = eliminate(joinedFactor, elimVar)
            factors = factorsWithoutEliVar
            factors.append(reduced)

        resFactor = joinFactors(factors)
        normalized = normalize(resFactor)
        return normalized
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            ...
            self.beliefs[p] = prior * likelihood
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            # 优化：只有当旧位置有概率时才进行扩散
            if oldProb > 0:
                # 获取转移概率分布 P(newPos | oldPos)
                # newPosDist 是一个字典: {位置A: 0.8, 位置B: 0.2 ...}
                newPosDist = self.getPositionDistribution(gameState, oldPos)

                for newPos, transitionProb in newPosDist.items():
                    newBeliefs[newPos] = (
                        newBeliefs.get(newPos, 0) + oldProb * transitionProb
                    )
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            pos = self.legalPositions[i % numLegalPositions]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.particles:
            ...
            # 累加权重 (DiscreteDistribution 本质是字典，可以直接 +=)
            # 注意：这会自动处理多个粒子在同一位置的情况
            weights[p] += prob

        # 2. 处理特殊情况：所有粒子权重为0 (跟丢了)
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return

        # 3. 重采样 (Resampling)
        # 根据权重分布，重新抽取 self.numParticles 个粒子
        newParticles = []
        for _ in range(self.numParticles):
            # sample() 会根据 value 的大小作为概率来返回 key (位置)
